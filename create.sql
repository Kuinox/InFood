...
CREATE TABLE IF NOT EXISTS `infood`.`grade_nutriment` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_grade_nutriment_UNIQUE` (`id` ASC),
  UNIQUE INDEX `tag_grade_nutriment_UNIQUE` (`label` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `infood`.`aliment` (
  ...
  CONSTRAINT `fk_aliment_grade_nutriment1`
    ...
    REFERENCES `infood`.`grade_nutriment` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `infood`.`manufacturing_place` (
  ...
  UNIQUE INDEX `name_manufacturing_place_UNIQUE` (`label` ASC),
...
CREATE TABLE IF NOT EXISTS `infood`.`allergen` (
  ...
  UNIQUE INDEX `name_allergen_UNIQUE` (`label` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `infood`.`categorie` (
  ...
  `tag_categorie` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id_categorie`),
  UNIQUE INDEX `id_categorie_UNIQUE` (`id_categorie` ASC),
  UNIQUE INDEX `tag_categorie_UNIQUE` (`tag_categorie` ASC))
ENGINE = InnoDB;
